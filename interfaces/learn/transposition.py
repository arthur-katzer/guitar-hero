"""Semitone transposition policy for Learn charts.

Learn parses MIDI files as candidate chart material. This module copies parsed
targets into corrected Learn expectations without modifying the source MIDI
...
from interfaces.learn.midi_targets import midi_note_name, required_match_ratio_for_note_count
from interfaces.learn.model import LearnSection, LearnTarget
...
class GuitarRangeValidation:
    """Result of validating a Learn chart against the project guitar range.

    The warning is based on active Learn expectations, not raw MIDI notes,
    because selected track plus transpose is the chart the user is practicing.

    @author Codex - created Learn guitar-range validation result.
    """
...
def clamp_transpose(semitones: int) -> int:
    """Return a user-controlled transpose value inside Learn's supported range.
    ...
    @author Codex - created Learn transpose bounds policy.
    """
...
def transpose_midi_note(original_midi_note: int, semitones: int) -> int:
    ...
    This is the core chart-correction rule: Learn expectations move by
    semitone offset while source MIDI note data remains unchanged.

    @author Codex - created Learn semitone transposition rule.
    """

    return int(original_midi_note) + int(semitones)


def apply_transpose(targets: Iterable[LearnTarget], semitones: int) -> list[LearnTarget]:
    """Return copied Learn targets with active notes shifted by semitones.
    ...
    @author Codex - created Learn target transposition policy.
    """
...
def transpose_section(section: LearnSection, semitones: int) -> LearnSection:
    """Return a section whose targets use transposed Learn expectations.

    Region and playback timing stay in seconds, so section bounds are copied
    exactly and only target pitches are replaced.

    @author Codex - created Learn section transposition policy.
    """
...
def note_range_for_targets(targets: Iterable[LearnTarget], *, original: bool = False) -> tuple[int, int] | None:
    """Return the lowest and highest note for raw or active Learn targets.

    Raw ranges explain the imported MIDI candidate. Active ranges explain what
    Learn will display and match after transpose.

    @author Codex - created Learn target range policy.
    """
...
def validate_guitar_range(targets: Iterable[LearnTarget]) -> GuitarRangeValidation:
    """Validate active Learn target notes against the project guitar range.
    ...
    @author Codex - created Learn guitar-range validation policy.
    """
...
def auto_suggest_transpose(targets: Iterable[LearnTarget]) -> int | None:
    """Suggest an upward transpose when the raw chart is just below E2.

    Learn does not auto-apply this suggestion because a low note may mean bass,
    ...
    @author Codex - created Learn transpose suggestion policy.
    """
...
def _target_with_transpose(target: LearnTarget, semitones: int) -> LearnTarget:
