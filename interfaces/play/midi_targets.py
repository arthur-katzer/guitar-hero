"""MIDI-to-PlayTarget extraction.

This module converts adapter-level MIDI files into Play domain objects. The
...
class MidiNoteEvent:
    ...
    The parser keeps note events separate from Play targets so grouping
    tolerance can evolve without changing raw MIDI extraction.

    @author Codex - created Play MIDI note event model.
    """
...
def midi_note_name(midi_note: int) -> str:
    ...
    Play shows expected targets as note names, while matching still uses MIDI
    numbers to avoid enharmonic spelling decisions in the first pass.

    @author Codex - created Play MIDI note naming.
    """
...
def group_note_events(
    ...
    *,
    ...
    """Group nearby MIDI starts into Play targets.
    ...
    @author Codex - created Play MIDI note grouping.
    """
...
def section_from_targets(targets: Iterable[PlayTarget]) -> PlaySection:
    """Create a Play section from already grouped targets.

    The demo path and MIDI parser both use this factory so controller tests see
    the same section bounds that the UI receives.

    @author Codex - created Play section factory from targets.
    """

    target_list = sorted(targets, key=lambda target: target.start_time)
    if not target_list:
        return PlaySection(start_time=0.0, end_time=0.0, targets=[])
    start_time = min(target.start_time for target in target_list)
    end_time = max(target.end_time for target in target_list)
    return PlaySection(start_time=start_time, end_time=end_time, targets=target_list)


def demo_song() -> PlaySong:
    """Return the built-in Play fallback sequence.

    Play must be usable even when MIDI parsing fails. The demo still goes
    through the real target model instead of creating a widget-only mock.

    @author Codex - created Play fallback demo song.
    """
...
def discover_midi_songs(project_root: Path) -> list[Path]:
    """Return bundled MIDI files that Play can offer by default.
    ...
    current menu visualizer assets predate the Play song library boundary.

    @author Codex - created Play bundled MIDI discovery.
...
def load_midi_song(
    ...
    *,
    ...
    try:
        ...
        raise RuntimeError("mido is required to parse MIDI files for Play mode") from exc
...
def _track_color(track_index: int) -> str:
    ...
    @author Codex - added deterministic Play track colors.
    """
...
def _program_labels(track: object) -> tuple[str, ...]:
    ...
    @author Codex - added Play track instrument labels.
    """
...
def _measure_marks_for_bounds(start_time: float, end_time: float, spacing_seconds: float = 1.5) -> tuple[MidiMeasureMark, ...]:
    """Return simple demo measure marks for non-file Play material.
...
def _beats_per_measure(midi_file: object) -> int:
    ...
    @author Codex - added Play piano-roll measure parsing.
    """
...
def _normalize_target_end_times(targets: list[PlayTarget]) -> list[PlayTarget]:
    """Keep target blocks visible without overstating their start timing.

    MIDI note-off data can be missing or extremely short. Play matching is
    based on target starts, so this only gives the timeline a practical block
    width.

    @author Codex - created Play target duration normalization.
    """

    normalized: list[PlayTarget] = []
...
def _tempo_map(midi_file: object) -> list[tuple[int, int]]:
    ...
    @author Codex - created Play MIDI tempo-map parser.
    """
...
def _seconds_at_tick(abs_tick: int, tempo_map: list[tuple[int, int]], ticks_per_beat: int) -> float:
    ...
    The conversion is local to Play extraction so controller timing never
    depends on mido objects.

    @author Codex - created Play MIDI tick conversion.
    """
...
def _track_name(track: object, *, fallback: str) -> str:
    ...
    @author Codex - created Play MIDI track label extraction.
    """
...
def _track_note_events(
    ...
    *,
    ...
    @author Codex - created Play MIDI note event extraction.
    """
