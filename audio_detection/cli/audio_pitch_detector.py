...
def write_wav(output_path: Path, sample_rate: int, mono: np.ndarray) -> None:
    """Write a diagnostic WAV only when recording mode actually needs SciPy.

    Realtime and diagnosis are the active terminal use cases in this project.
    Keeping SciPy out of module import lets those low-latency paths run with the
    current lightweight runtime while preserving the old recording feature for
    environments that install the optional WAV dependency.

    @author Codex - made SciPy lazy so live CLI modes can run from the root wrapper.
    """
    ...
    try:
        ...
        print(
            "error: scipy is required for --record WAV output. Run: python -m pip install scipy",
            file=sys.stderr,
        )
        raise SystemExit(1)
...
def record_audio(
    ...
    write_wav(output_path, sample_rate, mono)
