...
- `artifacts/505.wav`: WAV converted from a local 505 MP3 sample.
- `artifacts/505_chroma_report.csv`: frame-by-frame chroma similarity between
  the 505 audio and a matching 505 MIDI reference.
- `artifacts/505_vs_unrelated_report.csv`: the same audio compared against an
  unrelated MIDI reference as a negative control.
- `artifacts/505_chroma_similarity.png`: generated plot for the matching report.
- `artifacts/synthetic_a4.wav`: small generated audio smoke artifact.
...
- They made `artifacts/` look like canonical data instead of runtime output.
...
Constraints:
- Keep generated CSV, PNG, WAV, and JSON outputs under `artifacts/`.
