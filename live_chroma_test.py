...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Capture short microphone windows and compare them to target pitch classes."
    )
    parser.add_argument("--target", help="Target pitch classes, e.g. 0,4,7 or C,E,G")
    ...
    target_text = args.target or input("Target pitch classes (example 0,4,7 or C,E,G): ").strip()
    try:
        target_classes = parse_pitch_classes(target_text)
    ...
    try:
        while True:
            similarity, heard = capture_and_compare(
                sd,
                target,
